...
if not firebase_admin._apps:
    cred = credentials.Certificate(r"/Users/azizcan/Desktop/cs308-project/cs308-storewebsite-firebase-adminsdk-aswhm-5546b8efa9.json")
...
@app.post("/products/{objectId}/reviews/")
def add_review(
    objectId: str,
    review: ReviewBase,
...
@app.get("/products/{objectId}/reviews/", response_model=List[ReviewBase])
def get_reviews(objectId: str):
    try:
        # Access the product's review subcollection
        product_ref = db.collection("products").document(f"product{objectId}")
        review_collection = product_ref.collection("review")
        docs = review_collection.stream()
        ...
        # If no reviews found, raise a 404 error
        if not reviews:
            raise HTTPException(status_code=404, detail=f"No reviews found for product {objectId}")

        return reviews
    except Exception as e:
        print(f"Error fetching reviews for product {objectId}: {str(e)}")
        raise HTTPException(status_code=500, detail="Internal Server Error")
...
@app.get("/products/{objectId}/reviews", response_model=List[ReviewBase])
def get_product_reviews(objectId: str):
    """
    Fetch reviews for a specific product by objectId.
    """
    try:
        # Access the product's review subcollection
        product_ref = db.collection(f"products").document(f"product{objectId}")
        review_collection = product_ref.collection("review")  # Subcollection for reviews
        docs = review_collection.stream()
